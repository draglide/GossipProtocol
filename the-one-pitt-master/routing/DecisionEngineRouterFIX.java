 ...
 */
/*
 * When Connection goes up, a call to RoutingDecisionEngine.shouldDeleteMessage()
 * is made to prevent the message sent in other connection. So the message
 * can be deleted after no connection request it.
 */
public class DecisionEngineRouterFIX extends ActiveRouter
{
	...
	protected RoutingDecisionEngineFIX decider;
	...
	public DecisionEngineRouterFIX(Settings s)
	{
		super(s);
		...
		decider = (RoutingDecisionEngineFIX)routeSettings.createIntializedObject(
	...
	public DecisionEngineRouterFIX(DecisionEngineRouterFIX r)
	{
		super(r);
		outgoingMessages = new LinkedList<Tuple<Message, Connection>>();
	...
	{
		return new DecisionEngineRouterFIX(this);
	}
...
//	}
	...
	public void changedConnection(Connection con)
	{
		DTNHost myHost = getHost();
		DTNHost otherNode = con.getOtherNode(myHost);
		DecisionEngineRouterFIX otherRouter = (DecisionEngineRouterFIX)otherNode.getRouter();
		if(con.isUp())
		{
			decider.connectionUp(myHost, otherNode);
			/*
...
//                        }
	...
	{
		super.update();
                if (SimClock.getTime()-lastRecord>=UPDATE_INTERVAL) {
                    decider.update(getHost());
                    this.lastRecord = SimClock.getTime() - SimClock.getIntTime() % UPDATE_INTERVAL;
                }
	...
	public RoutingDecisionEngineFIX getDecisionEngine()
	{
		return this.decider;
	}
